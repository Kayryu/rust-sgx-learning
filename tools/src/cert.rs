...
impl<V> RaX509Cert<V> where V:AttestationReportVerifier {
    #[cfg(feature = "sgx")]
    pub fn generate(
    ...
    pub fn verify(cert: &[u8], now: u64) -> Result<ReportData, Error> {
        ...
        let sig_cert_dec = base64::decode_config(&report.cert_raw, base64::MIME).map_err(|_| Error::InvalidIASSigningCert)?;
        ...
        let ias_cert_dec = base64::decode_config(ias_ca_core, base64::MIME).map_err(|_| Error::InvalidIASSigningCert)?;

        let mut ca_reader = BufReader::new(&IAS_REPORT_CA[..]);

        let mut root_store = rustls::RootCertStore::empty();
        root_store.add_pem_file(&mut ca_reader).map_err(|_| Error::InvalidIASSigningCert)?;
