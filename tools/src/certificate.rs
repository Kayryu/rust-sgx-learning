...
mod tests {
    ...
    #[test]
    fn valid_cert_should_failed_with_mock() {
        let now = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();
        let ret = RaX509Cert::<MockVerifier>::verify(valid_cert, now);
        assert_eq!(ret, Err(Error::InvalidPublicKey));
    }

    #[test]
    fn valid_cert_should_failed_with_now() {
        let now = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();
        let ret = RaX509Cert::<Attestation>::verify(valid_cert, now);
        assert_eq!(ret, Err(Error::OutdatedReport));
    }

    #[test]
    fn valid_cert_should_ok_with_historical_time() {
        let time = 1640701728;
        let ret = RaX509Cert::<Attestation>::verify(valid_cert, time);
        assert!(ret.is_ok());
    }
}
